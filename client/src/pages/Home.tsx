...
export default function Home() {
  ...
  const catalogueSearch = trpc.catalogue.search.useQuery(
    { query: submittedQuery || "catalogue" },
    { enabled: hasSearched && !!submittedQuery && !isEmergency },
  );

  const resultKind = result?.type ?? searchType;
  const placeholder =
    searchType === "symptom"
    ...
    <div className="caresearch-shell">
      ...
      <header className="site-header">
        ...
        <nav className="desktop-nav" aria-label="Primary navigation">
          ...
          {user?.role === "admin" && <a href="/admin/medicines">Catalogue admin</a>}
        </nav>
        ...
        {menuOpen && (
          <div className="mobile-menu">
            ...
            {user?.role === "admin" && <a href="/admin/medicines" onClick={() => setMenuOpen(false)}>Catalogue admin</a>}
          </div>
        )}
      </header>
            ...
            <div id="search-result" className="result-area" aria-live="polite">
              {isEmergency ? (
                <div className="urgent-result">
                  ...
                  <img src="/manus-storage/caresearch-urgent-panel_1bbeed00.png" alt="Abstract protective safety beacon" />
                </div>
              ) : catalogueSearch.data?.length ? (
                <div className="catalogue-search-result">
                  <div className="catalogue-search-intro">
                    <div className="result-status"><Check size={14} aria-hidden="true" /> OWNER CATALOGUE MATCH</div>
                    <p className="eyebrow"><span /> APPROVED INFORMATION</p>
                    <h3>Items in the approved Medora catalogue match “{submittedQuery}”.</h3>
                    <p>These records match the search terms uploaded by the Medora owner. They are general information to review with a pharmacist or clinician, not a diagnosis, a personalized recommendation, or a dose.</p>
                  </div>
                  <div className="catalogue-search-list">
                    {catalogueSearch.data.map((item) => (
                      <article key={item.id} className="catalogue-search-card">
                        <div><p className="catalogue-card-class">{item.medicineClass}</p><h4>{item.genericName}</h4><p><b>Active ingredient:</b> {item.activeIngredient}</p></div>
                        <p className="catalogue-card-summary">{item.informationSummary}</p>
                        <div className="catalogue-card-safety"><ShieldAlert size={16} aria-hidden="true" /><span>{item.safetyNote}</span></div>
                        <div className="catalogue-card-footer"><span>Reviewed by {item.reviewerName} · {new Date(item.reviewedAt).toLocaleDateString()}</span><a href={item.sourceUrl} target="_blank" rel="noreferrer">Source <ExternalLink size={14} /></a></div>
                      </article>
                    ))}
                  </div>
                </div>
        ...
        <section className="medicine-section" id="medicine-info" aria-labelledby="medicine-title">
          <div className="medicine-illustration">
            <div className="image-card">
              <img src="/manus-storage/caresearch-detail-ingredients_de2246c3.png" alt="Editorial illustration of a medicine information card and magnifying glass" />
            </div>
      ...
      <footer className="site-footer">
        <div className="footer-main">
          <a className="brand-lockup medora-brand footer-brand" href="#top" aria-label="Medora home">
            <img src="/manus-storage/medora-wordmark_66812238.png" alt="Medora — Search. Find. Feel Better." />
            <span className="sr-only">Medora</span>
          </a>
